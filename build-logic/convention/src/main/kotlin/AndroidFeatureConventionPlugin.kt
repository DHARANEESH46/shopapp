import com.android.build.gradle.LibraryExtension
import org.gradle.api.Plugin
import org.gradle.api.Project
import org.gradle.kotlin.dsl.configure
import org.gradle.kotlin.dsl.dependencies

class AndroidFeatureConventionPlugin : Plugin<Project> {
    override fun apply(target: Project) {
        with(target) {
            with(pluginManager) {
                apply("shopapp.android.library")
                apply("shopapp.android.hilt")
                apply("org.jetbrains.kotlin.plugin.compose")
            }
            extensions.configure<LibraryExtension> {
                buildFeatures {
                    compose = true
                }
            }
            dependencies {
                add("implementation", "androidx.compose.ui:ui:1.7.6")
                add("implementation", "androidx.compose.material3:material3:1.3.1")
                add("implementation", "androidx.compose.ui:ui-tooling-preview:1.7.6")
                add("implementation", "androidx.lifecycle:lifecycle-viewmodel-compose:2.8.7")
                add("implementation", "androidx.lifecycle:lifecycle-runtime-compose:2.8.7")
                add("implementation", "androidx.hilt:hilt-navigation-compose:1.2.0")
                add("debugImplementation", "androidx.compose.ui:ui-tooling:1.7.6")
            }
        }
    }
}